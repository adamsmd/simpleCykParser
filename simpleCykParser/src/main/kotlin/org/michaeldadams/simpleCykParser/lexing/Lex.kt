 ...
 *
 * @receiver the rules to use for lexing
 ...
 */
fun LexRules.lex(chars: CharSequence): Pair<Int, List<Token>> {
  ...
  while (true) {
    ...
    pos = whitespace.regex.matchAt(chars, pos)?.let { it.range.endInclusive + 1 } ?: pos

    val token = terminalRules
